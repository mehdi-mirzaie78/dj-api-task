...
class EstimateEquipment(models.Model):
    estimate = models.ForeignKey(Estimate, on_delete=models.CASCADE, null=False, blank=False)
    equipment = models.ForeignKey(Equipment, on_delete=models.CASCADE, null=False, blank=False)
    quantity = models.PositiveIntegerField(null=False, blank=False)
    price_override = models.DecimalField(max_digits=8, decimal_places=2, null=False, blank=False)
    created_on = models.DateTimeField(auto_now_add=True)
